...
// Table structure elements may only contain table-section/row/cell elements,
// never phrasing content. Wrapping their (whitespace) text nodes in <span>
// produces invalid HTML and a hydration error, so skip them.
const NON_PHRASING_TABLE_PARENTS = new Set(['table', 'thead', 'tbody', 'tfoot', 'tr']);
...
export function rehypeWrapWords() {
  return (tree: Root) => {
    visit(tree, ['text', 'element'], (node, index, parent) => {
      ...
      if (parent.type === 'element' && NON_PHRASING_TABLE_PARENTS.has(parent.tagName)) return;
