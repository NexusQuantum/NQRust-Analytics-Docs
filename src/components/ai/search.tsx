...
import { Loader2, MessageCircleIcon, RefreshCw, SearchIcon, Send, Sparkles, User, X } from 'lucide-react';
...
export function AISearchPanelHeader({ className, ...props }: ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className="px-3 py-2 flex-1 flex items-center gap-2.5">
        <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-fd-primary text-fd-primary-foreground">
          <Sparkles className="size-4" />
        </div>
        <div className="min-w-0">
          <p className="text-sm font-medium">NQRust Analytics Assistant</p>
          <p className="text-xs text-fd-muted-foreground">
            AI can be inaccurate — please verify answers.
          </p>
        </div>
      </div>
...
const roleName: Record<string, string> = {
  user: 'You',
  assistant: 'NQRust Analytics Assistant',
};
  ...
  return (
    <div
      className={cn(
        ...
        isUser ? 'bg-fd-primary text-fd-primary-foreground' : 'bg-fd-secondary text-fd-primary',
      )}
    >
...
function Message({ message, ...props }: { message: ChatUIMessage } & ComponentProps<'div'>) {
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex min-w-0 flex-col gap-1', isUser ? 'items-end max-w-[85%]' : 'max-w-[90%]')}>
        <p
          className={cn(
            'text-xs font-medium text-fd-muted-foreground',
            !isUser && 'text-fd-primary',
          )}
        >
          ...
          <div
            className={cn(
              ...
              isUser
                ...
                : 'rounded-tl-sm border bg-fd-secondary text-fd-secondary-foreground',
            )}
          >
            {isUser ? (
              <p className="whitespace-pre-wrap break-words">{markdown}</p>
            ) : (
        ...
        {searchCalls.map((call) => (
          <div
            key={call.toolCallId}
            className="flex flex-row gap-2 items-center rounded-lg border bg-fd-secondary text-fd-muted-foreground text-xs p-2"
          >
